...
def generateDataMapAirSim(folders):
    ...
    for folder in folders:
        ...
        i=0
        while current_df['Throttle'][i+1]<.5:
        ...
        while current_df['Speed'][i-1]==0.0:
            i=i-1

        current_df=current_df.drop(list(range(i+10,len(current_df)-1)))
        current_df=current_df.reset_index()



        for i in range(1, current_df.shape[0] - 1, 1):
            previous_state = list(current_df.iloc[i - 1][['Steering', 'Throttle', 'Brake', 'Speed','POS_X','POS_Y','Q_W','Q_X','Q_Y','Q_Z']])

            if steering_wheel:
                current_label = list(((abs(current_df.iloc[i][['Steering']])/2.5+1).item().round(0), current_df.iloc[i][['Throttle']]))
            else:
                current_label = list((abs(current_df.iloc[i][['Steering']]*2+1).item().round(0), current_df.iloc[i][['Throttle']]+1))
...
def generatorForH5py(data_mappings, chunk_size=32):
    ...
    for chunk_id in range(0, len(data_mappings), chunk_size):
        # Data is expected to be a dict of <image: (label, previous_state)>
        # Extract the parts
...
def cook(folders, output_directory, train_eval_test_split):
    ...
    """
    output_files = [os.path.join(output_directory, f) for f in ['train.h5']]
