...
class ReferencePath:
    ...
    def compute_speed_profile(self, Constraints,init_velocity):
        """
        ...
        #v_min[0]=init_velocity-.1
        v_max = np.ones(N) * Constraints['v_max']
        #v_max[0]=init_velocity+.1
        ...
        self.waypoints[0].v_ref=init_velocity
...
if __name__ == '__main__':
    ...
    if path == 'Sim_Track':
        ...
        # Load map file
        map = Map(file_path='maps/env_1.png', origin=[ -70,-70],
                  resolution=1)

        ## env0 and env1 are -70 to 90

        # Specify waypoints
        with open('C:\\Users\\core-robotics\\Airsim_Lfd\\MINDMELD\\Car\\Data\\MeLD\\Pretest\\Ground_Truths\\env_0\\roll_out_2\\rrt_path100','rb') as f:
        ...
        for i in range(0,len(waypoints),20):
            wp_x.append(waypoints[i][0])
            wp_y.append(waypoints[i][1])
        print(wp_x,wp_y)

        base_path = 'C:/Users/core-robotics/AirSim_Lfd/MINDMELD/Car'
        path_save_gt_dir = os.path.join(base_path, 'Data', 'MeLD', 'Pretest', 'Ground_Truths', 'env_' + str(0),
                                        'roll_out_' + str(2))
        path_pretest = os.path.join(base_path, 'Data', 'MeLD', 'Pretest', 'Dagger_rollouts_rollouts', 'env_' + str(0),
                                    'roll_out_' + str(2), 'states_actions.pkl')
        ...
        reference_path = ReferencePath(map, wp_x, wp_y, path_resolution,
                     smoothing_distance=1, max_width=30,
                                       circular=False)
    ...
    reference_path.compute_speed_profile(SpeedProfileConstraints,init_velocity=state[3])
    # Get x and y locations of border cells for upper and lower bound
