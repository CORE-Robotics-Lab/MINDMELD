...
from MINDMELD.Car.MeLD.MeLD import Meld_Agent
import tkinter
import pandas as pd



class Meld_Pretest_TK():
    def __init__(self, num_env, num_rollouts, subj_id):
        ...
        self.pretest_agent = Pretest(self.car, 0, 0, subj_num=self.subj_num)
        self.i = 0
        self.j = 0
    ...
    def start(self):
        ...
        text_ready, text_go, text_finished = self.pretest_agent.record_corrective(self.top, env_num=self.i,
                                                                                  rollout_num=self.j)
        text_ready.destroy()
        text_go.destroy()
        self.startButton.configure(bg="green")

        if self.i == self.num_env-1 and self.j == self.num_rollouts-1:
            self.top.destroy()
        if self.j == self.num_rollouts - 1:
            self.j = 0
            self.i += 1
        else:
            self.j += 1
...
class Meld_Main_TK():
    def __init__(self, num_env, num_corrective, num_initial, subj_id):
        ...
        self.subj_num = subj_id
        self.agent = Meld_Agent(self.car, 0, 0, subj_num=self.subj_num)
        self.agent.train_test_mapper(train_test="train_one")
    ...
    def start(self):
        ...
        if self.train_flag:
            ...
            if self.i == self.num_env-1 and self.k == self.num_corrective:
                ...
                text_ready = tkinter.Label(self.top, text="Finished Training Agent", font=("Arial", 35), fg='red')
...
def pretrain_model():
    ...
    agent = Meld_Agent(car, 0, 0, subj_num=2000)

    agent.train_test_mapper(train_test="train")
